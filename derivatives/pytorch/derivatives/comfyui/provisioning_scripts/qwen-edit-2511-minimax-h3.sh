#!/bin/bash
#
# Qwen-Image-Edit-2511 (identity-preserving image edit) + MiniMax-H3 image-to-video.
#
#   https://docs.comfy.org/tutorials/image/qwen/qwen-image-edit
#   https://docs.comfy.org/tutorials/video/minimax/minimax-h3
#
# This is the pair the Novia background-card pipeline needs: Edit-2511 puts the
# character's own avatar into a new scene (it takes image inputs, so identity is
# carried by the reference rather than by the prompt), then H3 animates the result.
# Qwen-Image-2512 is deliberately NOT here - it is text-to-image with no image
# input, so it cannot preserve an identity and is only useful for a fresh avatar.
#
# Every node both templates use is comfy-core, so no custom nodes are cloned.
# H3 needs ComfyUI >= 0.30.0, which the image's COMFYUI_REF satisfies.
#
# Disk: ~78GB of weights, so give the instance 130GB+.
#
# The H3 text encoder is nvfp4, so this wants a Blackwell card (RTX 5090 / B200).
# On Volta or Turing it will not run at all; on Ada it falls back and is slow.
set -euo pipefail
...
HF_MODELS=(
    ...
    # fp8mixed unet: runs on Ada and Blackwell. Filenames are the ones the
    # template loads by name - do not rename them.
    ...
    |$MODELS_DIR/diffusion_models/qwen_image_edit_2511_fp8mixed.safetensors"
    ...
    # 4-step Lightning LoRA. A different repo from the older Qwen-Image-Lightning.
...
WGET_DOWNLOADS=(
    ...
    # --- Qwen-Image-Edit-2511 template and the two images it opens with ---
    "https://raw.githubusercontent.com/Comfy-Org/workflow_templates/refs/heads/main/templates/image_qwen_image_edit_2511.json
    |$WORKFLOWS_DIR/image_qwen_image_edit_2511.json"

    "https://raw.githubusercontent.com/Comfy-Org/workflow_templates/refs/heads/main/input/leather_sofa.png
    |$INPUTS_DIR/leather_sofa.png"

    "https://raw.githubusercontent.com/Comfy-Org/workflow_templates/refs/heads/main/input/texture_fur.png
    |$INPUTS_DIR/texture_fur.png"
...
main() {
    log "Starting Qwen-Image-Edit-2511 + MiniMax-H3 provisioning..."
